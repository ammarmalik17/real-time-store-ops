"""
Comprehensive OpenVINO optimization demo for YOLO models
Demonstrating latency vs throughput modes and async inference
"""
...
def get_optimal_num_requests(compiled_model):
    ...
    """
    try:
        # Try to get the optimal number of requests from the compiled model
        # This is the recommended approach from OpenVINO documentation
...
def main():
    parser = argparse.ArgumentParser(description="OpenVINO YOLO Optimization Demo")
    parser.add_argument("--mode", choices=["latency", "throughput", "async", "compare"], 
                        default="compare", help="Optimization mode to run")
    parser.add_argument("--model-path", default="openvino_conversion/models/yolo11n",
                        help="Path to the OpenVINO model")
    parser.add_argument("--benchmark", action="store_true",
                        help="Run benchmark instead of real-time inference")
    parser.add_argument("--iterations", type=int, default=50,
                        help="Number of iterations for benchmarking")
    
    args = parser.parse_args()
    
    print("OpenVINO YOLO Optimization Demo")
    print("="*50)
    ...
    if args.benchmark:
        ...
        if args.mode in ["latency", "compare"]:
            print("\n1. Testing LATENCY optimization mode...")
            latency_model = OpenVINOYOLO(model_path, optimization_mode="latency")
            latency_time, latency_fps, latency_detections = benchmark_model(
                latency_model, test_images, args.iterations)
        
        if args.mode in ["throughput", "compare"]:
            print("\n2. Testing THROUGHPUT optimization mode...")
            throughput_model = OpenVINOYOLO(model_path, optimization_mode="throughput")
            throughput_time, throughput_fps, throughput_detections = benchmark_model(
                throughput_model, test_images, args.iterations)
        
        if args.mode in ["async", "compare"]:
            print("\n3. Testing ASYNC THROUGHPUT optimization mode...")
            async_model = AsyncOpenVINOYOLO(model_path, optimization_mode="throughput")
            async_time, async_fps, async_detections = benchmark_async_model(
                async_model, test_images, args.iterations)
        
        if args.mode == "compare":
            print("\n" + "="*70)
            print("OPTIMIZATION MODE COMPARISON")
            print("="*70)
            if 'latency_fps' in locals():
                print(f"Latency Mode     - FPS: {latency_fps:.2f}, Time: {latency_time:.4f}s per inference")
            if 'throughput_fps' in locals():
                print(f"Throughput Mode  - FPS: {throughput_fps:.2f}, Time: {throughput_time:.4f}s per inference")
            if 'async_fps' in locals():
        ...
        if args.mode == "latency":
            model = OpenVINOYOLO(model_path, optimization_mode="latency")
            mode_name = "Latency Optimized"
        elif args.mode == "throughput":
            model = OpenVINOYOLO(model_path, optimization_mode="throughput")
            mode_name = "Throughput Optimized"
        elif args.mode == "async":
            model = AsyncOpenVINOYOLO(model_path, optimization_mode="throughput")
            mode_name = "Async Throughput Optimized"
        else:  # compare - default to throughput
            model = OpenVINOYOLO(model_path, optimization_mode="throughput")
            mode_name = "Throughput Optimized"
        ...
        print(f"Running real-time inference in {mode_name} mode")
        ...
        while True:
            ...
            if args.mode == "async":
                # For async demo, we'll run sync for display purposes
            ...
            cv2.putText(frame, f'FPS: {fps:.2f} ({mode_name})', (10, 30), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            
            cv2.imshow(f'OpenVINO YOLO - {mode_name}', frame)
